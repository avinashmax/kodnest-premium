/**
 * Job Notification Tracker - Status Tracking & Notifications
 * Version 5.0
 */

// --- STATE ---
let savedJobIds = JSON.parse(localStorage.getItem('savedJobs')) || [];
let jobStatus = JSON.parse(localStorage.getItem('jobTrackerStatus')) || {}; // { id: 'Applied' }
let activityLog = JSON.parse(localStorage.getItem('jobTrackerActivity')) || []; // [{ id, status, date, title, company }]
...
// --- ROUTER ---
const routes = { '/': 'Landing', '/dashboard': 'Dashboard', '/saved': 'Saved Jobs', '/digest': 'Daily Digest', '/settings': 'Settings', '/proof': 'Proof of Work' };
...
function renderLanding() {
    ...
    appContainer.innerHTML = `<div class="hero-section"><h1 class="hero-headline">Stop Missing The Right Jobs.</h1><p class="hero-subtext">Precision-matched job discovery delivered daily at 9AM.</p><a href="#/settings" class="btn btn-primary" style="font-size: 18px; padding: 16px 32px;">Start Tracking</a></div>`;
}
...
function renderDashboard() {
    ...
    let filtered = processed.filter(job => {
        ...
        const statusMatch = activeFilters.status === 'All' || currentStatus === activeFilters.status;
...
function renderProof() { appContainer.innerHTML = `<div class="settings-container"><h2 class="section-title">Proof of Implementation</h2><p class="form-hint">Status Tracking Logic Verified.</p></div>`; }
...
function savePreferences() {
    userPrefs = {
        roleKeywords: document.getElementById('prefRoles').value.split(',').filter(x => x.trim()),
        preferredLocations: document.getElementById('prefLocs').value.split(',').filter(x => x.trim()),
        skills: document.getElementById('prefSkills').value.split(',').filter(x => x.trim()),
...
function createFilterBar() {
    return `
        <div class="filter-bar">
            <div class="filter-group"><input type="text" id="filterKeyword" class="filter-input" placeholder="Search..." value="${activeFilters.keyword}"><label class="toggle-wrapper" title="Matches Only"><input type="checkbox" id="toggleMatch" class="toggle-checkbox" ${activeFilters.showMatchesOnly ? 'checked' : ''}><div class="toggle-switch"></div><span>Matches Only</span></label></div>
            <div class="filter-group">
                <select id="filterStatus" class="filter-select">
                    ...
                    <option value="Selected" ${activeFilters.status === 'Selected' ? 'selected' : ''}>Selected</option>
                </select>
                <select id="filterLoc" class="filter-select"><option value="All">Location: All</option><option value="Bangalore" ${activeFilters.location === 'Bangalore' ? 'selected' : ''}>Bangalore</option><option value="Pune" ${activeFilters.location === 'Pune' ? 'selected' : ''}>Pune</option><option value="Hyderabad" ${activeFilters.location === 'Hyderabad' ? 'selected' : ''}>Hyderabad</option><option value="Remote" ${activeFilters.location === 'Remote' ? 'selected' : ''}>Remote</option></select>
                <select id="filterSort" class="filter-select"><option value="Latest" ${activeFilters.sortBy === 'Latest' ? 'selected' : ''}>Sort: Latest</option><option value="Match Score" ${activeFilters.sortBy === 'Match Score' ? 'selected' : ''}>Sort: Match Score</option><option value="Salary" ${activeFilters.sortBy === 'Salary' ? 'selected' : ''}>Sort: Salary</option></select>
            </div>
        </div>
    `;
}
...
function createJobCard(job, isSavedView = false) {
    ...
    let badge = job.matchScore >= 80 ? `<span class="match-score score-high">⚡ ${job.matchScore}%</span>` : job.matchScore >= 60 ? `<span class="match-score score-mid">⚡ ${job.matchScore}%</span>` : `<span class="match-score score-neutral">⚡ ${job.matchScore}%</span>`;
    ...
    return `
        <div class="job-card">
            <div class="card-header"><div><div class="card-title">${job.title}</div><div class="card-company">${job.company}</div></div><div style="text-align:right;">${badge}<br><span class="badge-source badge-${job.source.toLowerCase()}">${job.source}</span></div></div>
            <div class="card-meta"><span class="meta-tag">📍 ${job.location}</span><span class="meta-tag">💼 ${job.experience}</span><span class="meta-tag salary">💰 ${job.salaryRange}</span><span class="meta-tag">🕒 ${job.postedDaysAgo}d ago</span></div>
...
function updateJobStatus(id, newStatus, title, company) {
    ...
    // Log Activity
    const logEntry = { id, status: newStatus, date: new Date().toISOString(), title, company };
    activityLog.unshift(logEntry);
    localStorage.setItem('jobTrackerActivity', JSON.stringify(activityLog));

    // Update UI (Quick Class Swap)
    renderDashboard(); // Re-render to update standard valid way, or just swap class for perf
...
function attachFilterListeners() {
    ...
    document.getElementById('filterStatus').addEventListener('change', (e) => { activeFilters.status = e.target.value; renderDashboard(); });
}

function toggleSave(id) { if (savedJobIds.includes(id)) savedJobIds = savedJobIds.filter(sid => sid !== id); else savedJobIds.push(id); localStorage.setItem('savedJobs', JSON.stringify(savedJobIds)); if (window.location.hash.slice(1) === '/saved') renderSavedJobs(); else if (window.location.hash.slice(1) === '/dashboard') renderDashboard(); }
function openModal(id) { const job = extendedJobData.find(j => j.id === id); modalBody.innerHTML = `<h2 style="font-family:var(--font-serif);font-size:24px;margin-bottom:8px;">${job.title}</h2><p style="margin-bottom:16px;">${job.company} • ${job.location}</p><p style="line-height:1.6;margin-bottom:24px;">${job.description}</p><div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:24px;">${job.skills.map(s => `<span class="meta-tag">${s}</span>`).join('')}</div><a href="${job.applyUrl}" target="_blank" class="btn btn-primary w-full" style="display:block;text-align:center;">Apply Now</a>`; modalOverlay.classList.add('open'); }
